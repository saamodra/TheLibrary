...
Public Class Kas
    ...
    Sub Sisa()
        Dim Str As String = "SELECT SUM(pemasukan) - SUM(pengeluaran) FROM tbl_kas"
        Dim cmd As New OleDb.OleDbCommand(Str, conn)
        RD = cmd.ExecuteReader()
        RD.Read()
        TextBox1.Text = FormatCurrency(RD.Item(0))
    End Sub
    Sub TampilGrid()
        Dim Str As String = "select * from [tbl_kas] where [tanggal] between #" & DateTimePicker1.Value.Month & "/" & DateTimePicker1.Value.Day & "/" & DateTimePicker1.Value.Year & "# and #" & DateTimePicker2.Value.Month & "/" & DateTimePicker2.Value.Day & "/" & DateTimePicker2.Value.Year & "#"
        Dim cmd As New OleDb.OleDbCommand(Str, conn)
        ...
        Do While dr.Read()
            Dim row As String()
            Dim i As Integer = 0
            row = New String() {dr.Item("id").ToString(), Format(dr.Item("tanggal"), "dd/MM/yyyy"), FormatCurrency(dr.Item("pemasukan")), FormatCurrency(dr.Item("pengeluaran")), dr.Item("keterangan").ToString(), dr.Item("operator").ToString()}
            DataGridView1.ColumnCount = 6
            DataGridView1.Columns(0).Name = "ID"
            DataGridView1.Columns(1).Name = "Tanggal"
            DataGridView1.Columns(2).Name = "Pemasukan"
            DataGridView1.Columns(3).Name = "Pengeluaran"
            DataGridView1.Columns(4).Name = "Keterangan"
            DataGridView1.Columns(5).Name = "Operator"
    ...
    Private Sub Kas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        e.Graphics.DrawString("Laporan Aliran Kas", New Font("Arial", 20), Brushes.Black, rect, sf)
        ...
        For p As Integer = startPage To pages.Count - 1
            ...
            Else
                startPage = 0
            End If
            e.Graphics.DrawString("Sisa Saldo = " & TextBox1.Text, New Font("Arial", 13), Brushes.Black, 500, startY + 20)
        Next
    End Sub
